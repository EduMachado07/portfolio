import {
  ArrowLeft,
  ArrowRight,
  CheckCircle2,
  Github,
  Lightbulb,
  Scale,
  XCircle,
  Youtube,
} from "lucide-react";
import { Link } from "react-router";
import { Button } from "@/components/ui/button";
import { GridBackground } from "@/components/grid-background";
import { Reveal } from "@/components/reveal";
import baquetasImg from "@/assets/bateria/img_baquetas.png";
import bateriaImg from "@/assets/bateria/img_bateriaCompleta.png";
import {
  GETTING_STARTED,
  GUIDE_VIDEO,
  HIGHLIGHTS,
  project,
  QUOTE,
  STACK_GROUPS,
  STRUCTURE_LAYERS,
  STRUCTURE_TREE,
  TIPS,
  TRADE_OFFS,
  WHEN_TO_AVOID,
  WHEN_TO_USE,
} from "./node-server-template.model";

export const NodeServerTemplatePage = () => {
  return (
    <div>
      <GridBackground cellWidth={52} cellHeight={52}>
        <header className="relative overflow-hidden px-[6%] py-16 md:px-[18%] md:py-20">
          <img
            src={baquetasImg}
            alt=""
            aria-hidden="true"
            className="pointer-events-none absolute -top-2 right-[8%] w-16 rotate-[18deg] object-contain opacity-[0.4] sm:right-[22%] sm:w-24"
          />
          <img
            src={bateriaImg}
            alt=""
            aria-hidden="true"
            className="pointer-events-none absolute -bottom-6 right-[18%] hidden w-50 rotate-[-6deg] object-contain opacity-[0.4] sm:block"
          />

          <Link
            to="/projetos"
            className="relative inline-flex items-center gap-1.5 text-sm font-medium text-zinc-500 transition-colors hover:text-zinc-950"
          >
            <ArrowLeft size={16} />
            Voltar para projetos
          </Link>

          <div className="relative mt-6 flex flex-wrap items-center gap-2 text-xs font-medium">
            <span className="rounded-md border bg-zinc-100 border-zinc-300 px-2 py-0.5 text-zinc-500">
              {project.purpose}
            </span>
            <span className="rounded-md border bg-zinc-100 border-zinc-300 px-2 py-0.5 text-zinc-500">
              {project.status}
            </span>
            <span className="rounded-md border bg-zinc-100 border-zinc-300 px-2 py-0.5 text-zinc-500">
              {project.systemType}
            </span>
          </div>

          <h1 className="relative mt-3 text-4xl font-bold text-zinc-950 sm:text-5xl">
            {project.title}
          </h1>

          <p className="relative mt-4 max-w-2xl text-base leading-relaxed text-zinc-600 sm:text-lg">
            {project.description}
          </p>

          <div className="relative mt-6 flex flex-wrap gap-3">
            <a href={project.repoUrl} target="_blank" rel="noopener noreferrer">
              <Button className="gap-2 bg-zinc-950 text-white hover:bg-zinc-800">
                <Github size={16} />
                Ver repositório
              </Button>
            </a>
          </div>
        </header>
      </GridBackground>

      <main className="mx-auto flex w-full flex-col gap-12 md:gap-20 px-[6%] py-6 md:py-16 md:px-[22%]">
        <Reveal className="grid grid-cols-1 md:grid-cols-3">
          <h2 className="text-sm font-semibold uppercase tracking-widest text-zinc-400 md:col-span-1">
            Por que existe
          </h2>
          <blockquote className="border-l-2 border-zinc-300 pl-5 text-lg leading-relaxed text-zinc-700 md:col-span-2">
            {QUOTE}
          </blockquote>
        </Reveal>

        <Reveal>
          <h2 className="mb-2 text-2xl font-bold text-zinc-950">Estrutura do projeto</h2>
          <p className="mb-6 text-sm leading-relaxed text-zinc-600">
            Cada caso de uso vive isolado na sua própria pasta, com o controller, o DTO e o
            teste logo ao lado — não tem "adivinhar onde fica" cada coisa.
          </p>
          <div className="overflow-hidden rounded-lg border border-zinc-200 bg-zinc-950 p-6">
            <pre className="overflow-x-auto text-xs leading-relaxed text-zinc-300 sm:text-sm">
              {STRUCTURE_TREE}
            </pre>
          </div>

          <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
            {STRUCTURE_LAYERS.map((layer) => (
              <div key={layer.name} className="rounded-lg border border-zinc-200 p-5">
                <h3 className="mb-1.5 font-mono text-sm font-semibold text-zinc-950">
                  {layer.name}
                </h3>
                <p className="text-sm leading-relaxed text-zinc-600">{layer.description}</p>
              </div>
            ))}
          </div>
        </Reveal>

        <Reveal>
          <h2 className="mb-2 text-2xl font-bold text-zinc-950">Quer usar no seu próximo projeto?</h2>
          <p className="mb-6 text-sm leading-relaxed text-zinc-600">
            É só clonar o repositório e seguir esses passos — em poucos minutos você tem uma
            API rodando, pronta pra receber o seu primeiro caso de uso.
          </p>
          <ol className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {GETTING_STARTED.map((item) => (
              <li key={item.step} className="rounded-lg border border-zinc-200 p-4">
                <p className="mb-1.5 text-xs font-semibold uppercase tracking-widest text-zinc-400">
                  {item.step}
                </p>
                <p className="text-sm leading-relaxed text-zinc-600">{item.description}</p>
              </li>
            ))}
          </ol>
          <a
            href={project.repoUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-6 inline-block"
          >
            <Button className="gap-2 bg-zinc-950 text-white hover:bg-zinc-800">
              <Github size={16} />
              Clonar repositório
            </Button>
          </a>
        </Reveal>

        <Reveal>
          <h2 className="mb-6 text-2xl font-bold text-zinc-950">Destaques técnicos</h2>
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            {HIGHLIGHTS.map((item) => {
              const Icon = item.icon;
              return (
                <div key={item.title} className="rounded-lg border border-zinc-200 p-5">
                  <div className="mb-3 flex size-9 items-center justify-center rounded-md bg-zinc-100">
                    <Icon size={18} />
                  </div>
                  <h3 className="mb-1.5 font-semibold text-zinc-950">{item.title}</h3>
                  <p className="text-sm leading-relaxed text-zinc-600">{item.description}</p>
                </div>
              );
            })}
          </div>
        </Reveal>

        <Reveal>
          <h2 className="mb-6 text-2xl font-bold text-zinc-950">Stack</h2>
          <div className="flex flex-wrap gap-1.5">
            {STACK_GROUPS[0].items.map((tech) => (
              <span
                key={tech}
                className="rounded-md bg-zinc-900 px-2.5 py-1 text-xs font-medium text-white"
              >
                {tech}
              </span>
            ))}
          </div>
        </Reveal>

        <Reveal>
          <h2 className="mb-2 text-2xl font-bold text-zinc-950">Quando vale usar essa estrutura</h2>
          <p className="mb-6 text-sm leading-relaxed text-zinc-600">
            Clean Architecture, DDD e SOLID não são "sempre certo" — são uma troca. Aqui vai a
            análise honesta de quando esse robustez compensa e quando ela só atrapalha.
          </p>

          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div className="rounded-lg border border-zinc-200 p-5">
              <div className="mb-3 flex items-center gap-2 text-emerald-600">
                <CheckCircle2 size={18} />
                <h3 className="font-semibold">Vale a pena quando...</h3>
              </div>
              <ul className="flex flex-col gap-2">
                {WHEN_TO_USE.map((item) => (
                  <li key={item} className="text-sm leading-relaxed text-zinc-600">
                    {item}
                  </li>
                ))}
              </ul>
            </div>

            <div className="rounded-lg border border-zinc-200 p-5">
              <div className="mb-3 flex items-center gap-2 text-red-500">
                <XCircle size={18} />
                <h3 className="font-semibold">É overkill quando...</h3>
              </div>
              <ul className="flex flex-col gap-2">
                {WHEN_TO_AVOID.map((item) => (
                  <li key={item} className="text-sm leading-relaxed text-zinc-600">
                    {item}
                  </li>
                ))}
              </ul>
            </div>
          </div>

          <div className="mt-6">
            <div className="mb-3 flex items-center gap-2">
              <Scale size={18} className="text-zinc-400" />
              <h3 className="font-semibold text-zinc-950">Trade-offs</h3>
            </div>
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
              {TRADE_OFFS.map((item) => (
                <div key={item.title} className="rounded-lg border border-zinc-200 p-5">
                  <h4 className="mb-1.5 font-semibold text-zinc-950">{item.title}</h4>
                  <p className="text-sm leading-relaxed text-zinc-600">{item.description}</p>
                </div>
              ))}
            </div>
          </div>

          <div className="mt-6 rounded-lg border border-zinc-200 bg-zinc-100 p-5">
            <div className="mb-3 flex items-center gap-2">
              <Lightbulb size={18} className="text-orange-500" />
              <h3 className="font-semibold text-orange-500">Dicas</h3>
            </div>
            <ul className="flex flex-col gap-2">
              {TIPS.map((tip) => (
                <li key={tip} className="text-sm leading-relaxed text-zinc-600">
                  {tip}
                </li>
              ))}
            </ul>
          </div>
        </Reveal>

        <Reveal>
          <h2 className="mb-2 text-2xl font-bold text-zinc-950">Vídeo que guiou essa arquitetura</h2>
          <p className="mb-6 text-sm leading-relaxed text-zinc-600">{GUIDE_VIDEO.description}</p>
          <div className="overflow-hidden rounded-lg border border-zinc-200 bg-zinc-950 shadow-sm">
            <div className="aspect-video w-full">
              <iframe
                className="h-full w-full"
                src={`https://www.youtube.com/embed/${GUIDE_VIDEO.youtubeId}`}
                title={GUIDE_VIDEO.title}
                loading="lazy"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                allowFullScreen
              />
            </div>
            <a
              href={GUIDE_VIDEO.watchUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 border-t border-zinc-800 px-4 py-3 text-xs font-medium text-zinc-400 transition-colors hover:text-zinc-200"
            >
              <Youtube size={13} />
              {GUIDE_VIDEO.title} — {GUIDE_VIDEO.channel}
            </a>
          </div>
        </Reveal>

        <Reveal className="overflow-hidden rounded-lg bg-zinc-800 opacity-10">
          <GridBackground cellWidth={32} cellHeight={32} style="opacity-10">
            <section className="flex flex-col items-center gap-4 px-6 py-10 text-center md:flex-row md:justify-between md:text-left">
              <div className="flex flex-col gap-1">
                <h2 className="text-xl font-bold text-white sm:text-2xl">Curtiu o que viu?</h2>
                <p className="text-sm text-zinc-400">
                  Bora conversar sobre o seu projeto e como posso te ajudar a tirá-lo do papel!
                </p>
              </div>
              <Link to="/contato" className="shrink-0">
                <Button className="gap-2 bg-white text-zinc-950 hover:bg-zinc-200">
                  Entrar em contato
                  <ArrowRight size={16} />
                </Button>
              </Link>
            </section>
          </GridBackground>
        </Reveal>
      </main>
    </div>
  );
};
