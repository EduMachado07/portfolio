import { ArrowLeft, ArrowRight, Construction, Github, Layers } from "lucide-react";
...
import { HIGHLIGHTS, project, QUOTE, STACK_GROUPS } from "./recommendation-system.model";

export const RecommendationSystemPage = () => {
  return (
    <div>
      ...
      <main className="mx-auto flex w-full flex-col gap-20 px-[6%] py-6 md:py-16 md:px-[22%]">
        <Reveal>
          <div className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 px-5 py-4 text-amber-800">
            <Construction size={20} className="mt-0.5 shrink-0" />
            <p className="text-sm font-medium leading-relaxed">
              Esta página ainda está em desenvolvimento — o projeto hoje é a
              documentação da arquitetura, a implementação ainda está por vir,
              e as imagens de tela ainda serão adicionadas.
            </p>
          </div>
        </Reveal>
        ...
        <Reveal>
          <div className="overflow-hidden rounded-lg border border-zinc-200 shadow-sm">
            <div className="flex aspect-video w-full items-center justify-center bg-zinc-200 text-zinc-400">
              <Layers size={32} />
            </div>
          </div>
        </Reveal>
