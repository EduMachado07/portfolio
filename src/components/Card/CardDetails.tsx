      ...
      <section className="h-max flex flex-col gap-2">
        ...
        <div className="flex gap-2">
          {type.slice(0, 4).map((tag, index) => (
            <div
              key={index}
              className="dark:text-zinc-50 dark:border-zinc-50 rounded-sm md:border-2 border px-2 py-0.5 md:text-sm text-xs font-supreme"
            >
              {tag}
            </div>
          ))}
        </div>
        <p className="dark:text-zinc-200 font-supreme max-md:text-sm line-clamp-3">{description}</p> {/* description */}
      </section>
      {/* link page project */}
      <section className="flex items-center justify-between">
        <div
          className="flex gap-1.5 items-center dark:text-zinc-50 rounded-sm md:text-base text-sm font-supreme"
        >
          <Calendar size={22} />
          {year}
        </div>
