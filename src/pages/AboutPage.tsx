...
const AboutPage = () => {
    ...
    return (
        <main className="px-[4vw] md:px-[12vw] flex-1 flex flex-col md:gap-8 gap-4">
            <h1 className="text-[#3399FF] font-chubbo-bold md:text-5xl text-4xl">Sobre mim</h1>
            <p className="dark:text-zinc-50 md:text-lg font-supreme md:w-4/5 text-pretty">
            ...
            <section className="dark:bg-[#222] rounded-lg overflow-hidden shadow-lg dark:shadow-zinc-900 md:p-8 p-4">
                {/* <img
                    src={imgPerfil}
                ...
                <p className="dark:text-zinc-200 md:text-lg font-supreme text-pretty mt-4">
            ...
            <h1 className="text-[#3399FF] font-chubbo-bold md:text-2xl text-xl">Formação Acadêmica</h1>
            <section className="relative flex flex-col gap-8 md:pl-14 pl-4 md:border-l-6 border-l-4 border-[#3399FF] pt-4 md:mx-20 mx-4">
                <AnimatePresence initial={false}>
                    {Schools.slice(0, numberSchools).map((school, index) => (
                        ...
                        >
                            {/* Marcador */}
                            <div className="absolute md:-left-19 -left-8 top-12 md:size-8 size-6 rounded-full bg-[#0d0d0d] md:border-4 border-3 border-[#f1f1f1] shadow-lg z-10" />
                            <div className="absolute md:-left-8 -left-0 md:top-16 top-14 w-2/4 border-[#f1f1f1] border-2 z-10" />

                            <main>
                                <div className="flex flex-col gap-2 text-zinc-50">
                                    <div className="mb-4">
                                        <h1 className="text-zinc-50 font-chubbo-bold md:text-lg">{school.startDate}</h1>
                                        <h1 className="text-[#3399FF] font-chubbo-bold md:text-xl text-base">{school.name}</h1>
                                    </div>
                                    <section className="flex max-md:flex-col gap-6">
                                        <img
                                            src={school.img}
                                            alt="Foto de Instituição"
                                            className="md:w-55 md:h-30 h-38 object-cover rounded-md"
                                        />
                                        <div className="flex flex-col gap-2">
                                            <div className="flex max-md:flex-col md:gap-6 gap-2 text-base font-supreme">
                                                <div className="max-md:text-sm flex items-center gap-1">
                                                    <Calendar size={20} /> {school.startDate} - {school.endDate}
                                                </div>
                                                <div className="max-md:text-sm flex items-center gap-1 text-pretty">
                                                    <MapPin size={20} /> {school.location}
                                                </div>
                                            </div>
                                            <p className="font-supreme md:text-lg">{school.description}</p>
                                        </div>
                                    </section>
                                </div>
                            </main>
