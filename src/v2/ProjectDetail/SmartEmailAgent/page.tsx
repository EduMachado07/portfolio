import { ArrowLeft, ArrowRight, Calendar, Github } from "lucide-react";
...
import {
  ...
  RESULTS_GALLERY,
  STACK_GROUPS,
} from "./smart-email-agent.model";

export const SmartEmailAgentPage = () => {
  return (
    <div>
      ...
      <main className="mx-auto flex w-full flex-col gap-20 px-[6%] py-16 md:px-[22%]">
        <Reveal>
          <div className="overflow-hidden rounded-lg border border-zinc-200 bg-zinc-950 shadow-sm">
            <img
              src={HERO_IMAGE}
              alt="Fluxo do agente no n8n: gatilho agendado, busca de e-mails, classificação por IA e ações finais"
              loading="lazy"
        ...
        <Reveal>
          <h2 className="mb-6 text-2xl font-bold text-zinc-950">Como funciona</h2>
        ...
        <Reveal>
          <h2 className="mb-6 text-2xl font-bold text-zinc-950">Resultado, na prática</h2>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div className="overflow-hidden rounded-lg border border-zinc-200 bg-zinc-950 shadow-sm">
              <img
                src={RESULTS_GALLERY.telegram.image}
                alt={RESULTS_GALLERY.telegram.alt}
                loading="lazy"
                className="w-full object-cover"
              />
              <p className="flex items-center gap-1.5 border-t border-zinc-800 px-4 py-3 text-xs font-medium text-zinc-400">
                <NOTIFY_ICON size={13} />
            ...
            <div className="overflow-hidden rounded-lg border border-zinc-200 bg-zinc-950 shadow-sm">
              <img
                src={RESULTS_GALLERY.calendar.image}
                alt={RESULTS_GALLERY.calendar.alt}
                loading="lazy"
                className="w-full object-cover"
              />
              <p className="flex items-center gap-1.5 border-t border-zinc-800 px-4 py-3 text-xs font-medium text-zinc-400">
                <Calendar size={13} />
                Evento criado sozinho no Google Calendar
              </p>
            </div>
          </div>
        </Reveal>
